module.exports = function (app, passport, db, multer, ObjectId) {
  ...
  app.get('/profile', isLoggedIn, function (req, res) {
    db.collection('posts').find({ caption: "asdflkjasdflkasjdf" }).toArray((err, result) => {
  ...
  app.get('/post/:zebra', isLoggedIn, function (req, res) {
    ...
    db.collection('posts').aggregate([
      ...
      if (err) return console.log(err)
      console.log(result)
      const registered = result[0].registeredUsers.some(user => String(user._id) == String(req.user._id))
      console.log('REGISTERED?:', result[0].registeredUsers, req.user._id, registered)
      db.collection('comments').find({ eventID: postId, postedBy: req.user._id }).toArray((err, comments) => {
        console.log(comments)

        res.render(String(result[0].postedBy) === String(req.user._id) ? 'edit.ejs' : 'post.ejs', {
          posts: result, comments, registered
        })
      })
  ...
  */
  ...
  app.post('/post/:id/comment', (req, res) => {
    ...
    db.collection("comments").insertOne({
      eventComment: req.body.eventComment, postedBy: req.user._id, eventID: postId,
    }, (err, result) => {
      if (err) return console.log(err)
      console.log('saved to database')
